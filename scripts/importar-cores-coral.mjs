// Importa o catálogo de cores da Coral (scripts/cores-coral.json) para a
// coleção Firestore `cores_coral`. Roda uma única vez, manualmente:
//
//   node scripts/importar-cores-coral.mjs
//
...
async function main() {
  console.log("=== Import do catálogo de cores Coral para Firestore ===\n");

  const jsonPath = join(__dirname, "cores-coral.json");
  const cores = JSON.parse(readFileSync(jsonPath, "utf8"));
  console.log(`Cores lidas de cores-coral.json: ${cores.length}\n`);
  ...
  const coresRef = collection(db, "cores_coral");

  console.log("Verificando cores já existentes na coleção `cores_coral`...");
  ...
  const confirma = await perguntaTexto(
    `Confirma gravar ${pendentes.length} documentos na coleção "cores_coral" do projeto "${firebaseConfig.projectId}"? (digite SIM para continuar): `
  );
  ...
  console.log(`\nImport concluído. ${gravados} cores gravadas na coleção "cores_coral".`);
  process.exit(0);
}
